...
const ENDPOINT_KEY = 'inpaint_endpoint_id'
const DEFAULT_ENDPOINT_ID = 'xio27s12llqzpa'
const RUN_ENDPOINT = '/api/blocks/inpaint/run'
const STATUS_ENDPOINT_BASE = '/api/blocks/inpaint/status'
...
function InpaintBlock({
  ...
  const [denoise, setDenoise] = useSessionState(`block_${blockId}_denoise`, 0.9)
  ...
  const { get: getBinding } = useBlockBindings(blockId, 'inpaint', inputs)
  const promptBinding = getBinding('prompt')
  const imageBinding = getBinding('image')
  const maskBinding = getBinding('mask')
  ...
  const isMaskWired = Boolean(maskBinding?.usesUpstreamAtRuntime)
  const maskUrl = String(maskBinding?.value ?? '')
  ...
  useEffect(() => {
    registerExecute(async (freshInputs) => {
      ...
      const runMask = isMaskWired ? String(freshInputs.mask ?? '') : String(maskBinding?.localValue ?? '')

      if (!runPrompt.trim()) throw new Error('Prompt is required')
      if (!runImage.trim()) throw new Error('Source image URL is required')
      if (!runMask.trim()) throw new Error('Mask URL is required (PNG with red channel = inpaint area)')
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <Label className="text-xs">Mask (red = regenerate)</Label>
          <Select
            value={maskBinding?.selectedSourceValue || MANUAL_SOURCE}
            onValueChange={(v) => maskBinding?.setSelectedSource?.(v)}
          >
            <SelectTrigger className="h-7 min-w-[170px] text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {(maskBinding?.sourceOptions ?? []).map((o) => (
                <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        {!isMaskWired && (
          <UrlOrFileInput
            value={maskUrl}
            onChange={(v) => maskBinding?.setLocalValue(v)}
            accept="image/*"
            placeholder="https://... or use Upload (PNG with red mask area)"
          />
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="space-y-1">
          <Label className="text-xs">Denoise <span className="text-[10px] text-muted-foreground font-normal">(0.9 typical)</span></Label>
...
export const blockDef: BlockDef = {
  ...
  label: 'Inpaint',
  description:
    'Illustrious XL inpaint — regenerates a masked area while preserving the rest. ' +
    'Mask is a PNG where red channel marks the region to regenerate. Denoise 0.9 = full repaint, lower = blend.',
  size: 'huge',
  canStart: false,
  inputs: [
    { name: 'image', kind: PORT_IMAGE, required: false },
    { name: 'mask', kind: PORT_IMAGE, required: false },
  ...
  bindings: [
    ...
    { field: 'mask', input: 'mask', mode: 'upstream_or_local', allowOverride: true },
