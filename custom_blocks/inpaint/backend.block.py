"""Inpaint — Illustrious XL inpaint block.

Thin wrapper around ``backend.m_routes.build_illustrious_inpaint_workflow``.
Same JSON output as the mobile ``/api/m/inpaint`` route.
"""
...
from backend.m_routes import NEGATIVE_DEFAULT, build_illustrious_inpaint_workflow
...
async def run(request: Request) -> JSONResponse:
    ...
    if not mask_url:
        return JSONResponse({"ok": False, "error": "mask_url is required"}, status_code=400)
    if not prompt:
    ...
    image_filename = "inpaint_src.png"
    mask_filename = "inpaint_mask.png"
