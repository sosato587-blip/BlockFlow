...
const RUN_ENDPOINT = '/api/blocks/wan_animate/run'
const STATUS_ENDPOINT_BASE = '/api/blocks/wan_animate/status'
...
function WanAnimateBlock({
  ...
  // Kijai Wan Animate defaults: 832x480 landscape, 81 frames @ 16 fps,
  // lightx2v acceleration LoRA included by default so steps stay at 6.
  ...
  const [cfg, setCfg] = useSessionState(`block_${blockId}_cfg`, 5.0)
  const [shift, setShift] = useSessionState(`block_${blockId}_shift`, 1.0)
  const [steps, setSteps] = useSessionState(`block_${blockId}_steps`, 6)
  const [scheduler, setScheduler] = useSessionState(`block_${blockId}_scheduler`, 'dpm++_sde')
  ...
  const { get: getBinding } = useBlockBindings(blockId, 'wanAnimate', inputs)
    ...
    registerExecute(async (freshInputs) => {
      ...
      const res = await fetch(RUN_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          prompt: runPrompt || 'character performing the driving motion, smooth animation, high quality',
          width,
          height,
          length,
          fps,
