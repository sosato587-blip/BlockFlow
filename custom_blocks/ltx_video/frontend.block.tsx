...
const ENDPOINT_KEY = 'ltx_video_endpoint_id'
const DEFAULT_ENDPOINT_ID = 'xio27s12llqzpa'
const RUN_ENDPOINT = '/api/blocks/ltx_video/run'
const STATUS_ENDPOINT_BASE = '/api/blocks/ltx_video/status'
...
function LtxVideoBlock({
  ...
  // LTX 0.9.5 defaults: 768x512 landscape, 97 frames @ 25 fps, 30 steps,
  // cfg 3.0. T2V is the default; supplying an upstream image (or pasting
  // a URL) flips the worker into I2V via LTXVImgToVideo. Width/height
  // must be divisible by 32; length should follow the 8n+1 rule (25,
  // 33, 41, 49, …, 97, 105, 121, 161).
  const [width, setWidth] = useSessionState(`block_${blockId}_width`, 768)
  const [height, setHeight] = useSessionState(`block_${blockId}_height`, 512)
  const [length, setLength] = useSessionState(`block_${blockId}_length`, 97)
  const [fps, setFps] = useSessionState(`block_${blockId}_fps`, 25)
  const [cfg, setCfg] = useSessionState(`block_${blockId}_cfg`, 3.0)
  ...
  const { get: getBinding } = useBlockBindings(blockId, 'ltxVideo', inputs)
  ...
  useEffect(() => {
    registerExecute(async (freshInputs) => {
      ...
      // image_url is optional: empty -> T2V, present -> I2V
      ...
      const jobId = String(data.remote_job_id || data.job_ids?.[0] || '').trim()
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        ...
        {!isImageWired && (
          <UrlOrFileInput
            ...
            accept="image/*"
            placeholder="(optional) https://... or use Upload — leave blank for T2V"
          />
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="space-y-1">
          <Label className="text-xs">FPS <span className="text-[10px] text-muted-foreground font-normal">(25 default)</span></Label>
          <Input type="number" min={8} step={1} value={fps}
            onChange={(e) => setFps(Number(e.target.value))} className="h-8 text-xs" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">CFG <span className="text-[10px] text-muted-foreground font-normal">(3.0 recommended)</span></Label>
          <Input type="number" min={0.1} step={0.5} value={cfg}
            onChange={(e) => setCfg(Number(e.target.value))} className="h-8 text-xs" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Steps <span className="text-[10px] text-muted-foreground font-normal">(30 typical)</span></Label>
...
export const blockDef: BlockDef = {
  ...
  label: 'LTX Video',
  description:
    'LTX 0.9.5 — fast / cheap text-to-video and image-to-video (~4-6× cheaper than Wan I2V). ' +
    'Leave the image input empty for T2V; supply an upstream image or paste a URL for I2V.',
  size: 'huge',
  canStart: false,
  inputs: [
